...
| Team | 4 FTE: 1 lead engineer, 2 engineers, 1 field technician (part-time). Client side: operations manager at 0.5 FTE, gate staff for acceptance testing. |
...
| Team | 5 FTE: Phase 0 team plus 1 data engineer; field technician full time during the sensor rollout (months 3-5). Client side: head keeper at 0.3 FTE for threshold definition and alert triage design. |
...
| Team | 6 FTE: previous team plus 1 ML engineer on a 6-month contract. This is the peak of the plan, and it is temporary by design: the steady-state operating team is 2 engineers (NFR-OPS-1). |
| CapEx delta | 5,000 EUR (assumption), training compute and a labelling workstation |
| OpEx delta | +550 EUR/month (assumption), of which about 250 is assistant inference |
...
| Team | 5 FTE: ML engineer contract reduced to part-time; one engineer transitions to the client's payroll as the future operator. Knowledge transfer is a deliverable of this phase, not an afterthought. |
| CapEx delta | 3,000 EUR (assumption) |
| OpEx delta | +250 EUR/month (assumption) |
...
| Team | 4 FTE, of which 2 are now the client's own staff. External involvement ends at month 36. |
| CapEx delta | 0 |
| OpEx delta | +200 EUR/month (assumption) |
...
| **Piranha tanks are not camera-observable** (A11 fails) | FR-9 as designed is infeasible. | Fall back to a sampling protocol with a documented count procedure and a manual entry screen, which is what the estate does today, plus scheduled reminders. Reassign the GPU node to welfare video if useful, otherwise do not buy it. | Saves about 8,000 EUR (assumption). FR-9 is met at lower quality, and we say so rather than pretending a model exists. |
