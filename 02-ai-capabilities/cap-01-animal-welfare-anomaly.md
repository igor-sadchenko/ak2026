...
| Per-animal identity (PIT or RFID tags) with per-individual baselines | The sharpest criticism of this design, and we take the trade knowingly. A per-enclosure baseline can be blind to one sick animal in a shared enclosure. Against that: tagging exotic and poisonous animals (F4) means handling them, which is the risk we are trying to reduce, and we are not willing to justify that on the strength of a model not yet shown to work. Sequence: prove the enclosure-level model, then revisit if its misses turn out to be individual-level ones. See [../05-process/decision-log.md](../05-process/decision-log.md) D12. |
