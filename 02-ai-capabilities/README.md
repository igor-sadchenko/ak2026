...
| Ride maintenance planning (F3) | Predictive maintenance on 18th-century rides | Fixed inspection intervals from the existing certification regime, plus a fault log | The rides are already certified under a regime with defined intervals (C5). There is no sensor history, the failure cost is safety-critical, and a model trained on 40 heterogeneous historic machines would be an unverifiable claim in the one area where claims must be verifiable. |
